import {FetchTypes, Query, QueryBuilder, QueryBuilderOptions} from "workers-qb";
    ...
    return await this.loggerWrapper(query, this.options.logger, async () => {
      ...
      if (query.fetchType === FetchTypes.ONE || query.fetchType === FetchTypes.ALL) {
        ...
        return {
          ...
          results: query.fetchType === FetchTypes.ONE && result.length > 0 ? result[0] : result,
        }
      }
